...
module IncidentIoSdk
  class IncidentUpdatesV2Api
    ...
    def incident_updates_v2_list(opts = {})
      data, _status_code, _headers = incident_updates_v2_list_with_http_info(opts)
      data
    end
    ...
    # @option opts [Integer] :page_size Integer number of records to return (default to 25)
    # @option opts [String] :after An record&#39;s ID. This endpoint will return a list of records after this ID in relation to the API response order.
    # @return [Array<(IncidentUpdatesListResultV2, Integer, Hash)>] IncidentUpdatesListResultV2 data, response status code and response headers
    def incident_updates_v2_list_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: IncidentUpdatesV2Api.incident_updates_v2_list ...'
      end
      # resource path
      local_var_path = '/v2/incident_updates'
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'IncidentUpdatesListResultV2'

      # auth_names
      auth_names = opts[:debug_auth_names] || []

      new_options = opts.merge(
        :operation => :"IncidentUpdatesV2Api.incident_updates_v2_list",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: IncidentUpdatesV2Api#incident_updates_v2_list\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
