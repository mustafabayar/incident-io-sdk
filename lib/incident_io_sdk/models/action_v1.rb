  ...
  class ActionV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ActionV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ActionV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @follow_up.nil?
        invalid_properties.push('invalid value for "follow_up", follow_up cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @follow_up.nil?
      return false if @id.nil?
      return false if @incident_id.nil?
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["outstanding", "completed", "deleted", "not_doing"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] follow_up Value to be assigned
    def follow_up=(follow_up)
      if follow_up.nil?
        fail ArgumentError, 'follow_up cannot be nil'
      end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["outstanding", "completed", "deleted", "not_doing"])
    ...
    def hash
      [assignee, completed_at, created_at, description, external_issue_reference, follow_up, id, incident_id, status, updated_at].hash
    end
