  ...
  class AfterPaginationMetaResultV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AfterPaginationMetaResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AfterPaginationMetaResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'after_url')
        self.after_url = attributes[:'after_url']
      else
        self.after_url = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @after_url.nil?
        invalid_properties.push('invalid value for "after_url", after_url cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] after_url Value to be assigned
    def after_url=(after_url)
      if after_url.nil?
        fail ArgumentError, 'after_url cannot be nil'
      end
