...
module IncidentIoSdk
  class AlertAttributesListResultV2
    attr_accessor :alert_attributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertAttributesListResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertAttributesListResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alert_attributes')
        if (value = attributes[:'alert_attributes']).is_a?(Array)
          self.alert_attributes = value
        end
      else
        self.alert_attributes = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @alert_attributes.nil?
        invalid_properties.push('invalid value for "alert_attributes", alert_attributes cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @alert_attributes.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] alert_attributes Value to be assigned
    def alert_attributes=(alert_attributes)
      if alert_attributes.nil?
        fail ArgumentError, 'alert_attributes cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          alert_attributes == o.alert_attributes
    end
