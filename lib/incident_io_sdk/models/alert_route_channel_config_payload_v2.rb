...
module IncidentIoSdk
  class AlertRouteChannelConfigPayloadV2
    ...
    attr_accessor :ms_teams_targets

    attr_accessor :slack_targets
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRouteChannelConfigPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRouteChannelConfigPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ms_teams_targets')
        self.ms_teams_targets = attributes[:'ms_teams_targets']
      end

      if attributes.key?(:'slack_targets')
        self.slack_targets = attributes[:'slack_targets']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ms_teams_targets == o.ms_teams_targets &&
          slack_targets == o.slack_targets
    end
    ...
    def hash
      [condition_groups, ms_teams_targets, slack_targets].hash
    end
