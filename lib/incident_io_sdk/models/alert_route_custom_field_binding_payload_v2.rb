  ...
  class AlertRouteCustomFieldBindingPayloadV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRouteCustomFieldBindingPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRouteCustomFieldBindingPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @merge_strategy.nil?
        invalid_properties.push('invalid value for "merge_strategy", merge_strategy cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @merge_strategy.nil?
      merge_strategy_validator = EnumAttributeValidator.new('String', ["first-wins", "last-wins", "append"])
      return false unless merge_strategy_validator.valid?(@merge_strategy)
      true
    end
    ...
    def merge_strategy=(merge_strategy)
      validator = EnumAttributeValidator.new('String', ["first-wins", "last-wins", "append"])
      unless validator.valid?(merge_strategy)
        fail ArgumentError, "invalid value for \"merge_strategy\", must be one of #{validator.allowable_values}."
      end
      @merge_strategy = merge_strategy
    end
    ...
    def hash
      [binding, custom_field_id, merge_strategy].hash
    end
