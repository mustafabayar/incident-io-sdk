...
module IncidentIoSdk
  class AlertRouteEscalationTargetPayloadV2
    attr_accessor :escalation_paths
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRouteEscalationTargetPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRouteEscalationTargetPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'escalation_paths')
        self.escalation_paths = attributes[:'escalation_paths']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          escalation_paths == o.escalation_paths &&
          users == o.users
    end
