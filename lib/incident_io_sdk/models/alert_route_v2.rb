...
module IncidentIoSdk
  class AlertRouteV2
    ...
    def self.openapi_types
      {
        :'alert_sources' => :'Array<AlertRouteAlertSourceV2>',
        :'channel_config' => :'Array<AlertRouteChannelConfigV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRouteV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRouteV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [alert_sources, channel_config, condition_groups, created_at, enabled, escalation_config, expressions, id, incident_config, incident_template, is_private, name, updated_at, version].hash
    end
