  ...
  class AlertRoutesCreateResultV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRoutesCreateResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRoutesCreateResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alert_route')
        self.alert_route = attributes[:'alert_route']
      else
        self.alert_route = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @alert_route.nil?
        invalid_properties.push('invalid value for "alert_route", alert_route cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @alert_route.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] alert_route Value to be assigned
    def alert_route=(alert_route)
      if alert_route.nil?
        fail ArgumentError, 'alert_route cannot be nil'
      end
