...
module IncidentIoSdk
  class AlertRoutesListResultV2
    ...
    def self.openapi_types
      {
        :'alert_routes' => :'Array<AlertRouteSlimV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertRoutesListResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertRoutesListResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alert_routes')
        if (value = attributes[:'alert_routes']).is_a?(Array)
          self.alert_routes = value
        end
      else
        self.alert_routes = nil
      end
    ...
    def list_invalid_properties
      ...
      if @alert_routes.nil?
        invalid_properties.push('invalid value for "alert_routes", alert_routes cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @alert_routes.nil?
      return false if @pagination_meta.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] alert_routes Value to be assigned
    def alert_routes=(alert_routes)
      if alert_routes.nil?
        fail ArgumentError, 'alert_routes cannot be nil'
      end
    ...
    def hash
      [alert_routes, pagination_meta].hash
    end
