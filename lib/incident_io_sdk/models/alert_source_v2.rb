...
module IncidentIoSdk
  class AlertSourceV2
    ...
    def self.openapi_types
      {
        :'email_options' => :'AlertSourceEmailOptionsV2',
        :'id' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertSourceV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertSourceV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'jira_options')
        self.jira_options = attributes[:'jira_options']
      end
    ...
    def source_type=(source_type)
      validator = EnumAttributeValidator.new('String', ["alertmanager", "app_optics", "azure_monitor", "bugsnag", "checkly", "chronosphere", "cloudwatch", "cloudflare", "cronitor", "crowdstrike_falcon", "datadog", "dynatrace", "elasticsearch", "email", "expel", "github_issue", "google_cloud", "grafana", "http", "honeycomb", "incoming_calls", "jira", "monte_carlo", "nagios", "new_relic", "opsgenie", "prtg", "pager_duty", "panther", "pingdom", "runscope", "sns", "sentry", "splunk", "status_cake", "status_page_views", "sumo_logic", "uptime", "zendesk"])
    ...
    def hash
      [email_options, id, jira_options, name, secret_token, source_type, template].hash
    end
