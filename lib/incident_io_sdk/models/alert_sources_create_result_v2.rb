  ...
  class AlertSourcesCreateResultV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertSourcesCreateResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertSourcesCreateResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alert_source')
        self.alert_source = attributes[:'alert_source']
      else
        self.alert_source = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @alert_source.nil?
        invalid_properties.push('invalid value for "alert_source", alert_source cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @alert_source.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] alert_source Value to be assigned
    def alert_source=(alert_source)
      if alert_source.nil?
        fail ArgumentError, 'alert_source cannot be nil'
      end
