  ...
  class AlertV2
    ...
    def self.openapi_types
      {
        ...
        :'attributes' => :'Array<AlertAttributeEntryV2>',
        :'created_at' => :'Time',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'alert_source_id')
        self.alert_source_id = attributes[:'alert_source_id']
      else
        self.alert_source_id = nil
      end
    ...
    def list_invalid_properties
      ...
      if @alert_source_id.nil?
        invalid_properties.push('invalid value for "alert_source_id", alert_source_id cannot be nil.')
      end
      ...
      if @deduplication_key.nil?
        invalid_properties.push('invalid value for "deduplication_key", deduplication_key cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @alert_source_id.nil?
      return false if @attributes.nil?
      return false if @created_at.nil?
      return false if @deduplication_key.nil?
      return false if @id.nil?
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["firing", "resolved"])
    ...
    # @param [Object] alert_source_id Value to be assigned
    def alert_source_id=(alert_source_id)
      if alert_source_id.nil?
        fail ArgumentError, 'alert_source_id cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] deduplication_key Value to be assigned
    def deduplication_key=(deduplication_key)
      if deduplication_key.nil?
        fail ArgumentError, 'deduplication_key cannot be nil'
      end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["firing", "resolved"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          alert_source_id == o.alert_source_id &&
    ...
    def hash
      [alert_source_id, attributes, created_at, deduplication_key, description, id, resolved_at, source_url, status, title, updated_at].hash
    end
