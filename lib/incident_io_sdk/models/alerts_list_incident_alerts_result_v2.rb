...
module IncidentIoSdk
  class AlertsListIncidentAlertsResultV2
    attr_accessor :incident_alerts

    attr_accessor :pagination_meta
    ...
    def self.openapi_types
      {
        :'incident_alerts' => :'Array<IncidentAlertV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AlertsListIncidentAlertsResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AlertsListIncidentAlertsResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_alerts')
        if (value = attributes[:'incident_alerts']).is_a?(Array)
          self.incident_alerts = value
        end
      else
        self.incident_alerts = nil
      end

      if attributes.key?(:'pagination_meta')
        self.pagination_meta = attributes[:'pagination_meta']
      else
        self.pagination_meta = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_alerts.nil?
        invalid_properties.push('invalid value for "incident_alerts", incident_alerts cannot be nil.')
      end

      if @pagination_meta.nil?
        invalid_properties.push('invalid value for "pagination_meta", pagination_meta cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_alerts.nil?
      return false if @pagination_meta.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_alerts Value to be assigned
    def incident_alerts=(incident_alerts)
      if incident_alerts.nil?
        fail ArgumentError, 'incident_alerts cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] pagination_meta Value to be assigned
    def pagination_meta=(pagination_meta)
      if pagination_meta.nil?
        fail ArgumentError, 'pagination_meta cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_alerts == o.incident_alerts &&
          pagination_meta == o.pagination_meta
    end
    ...
    def hash
      [incident_alerts, pagination_meta].hash
    end
