  ...
  class AuditLogActorMetadataV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AuditLogActorMetadataV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AuditLogActorMetadataV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'api_key_roles')
        self.api_key_roles = attributes[:'api_key_roles']
      end

      if attributes.key?(:'external_resource_external_id')
        self.external_resource_external_id = attributes[:'external_resource_external_id']
      end

      if attributes.key?(:'external_resource_type')
        self.external_resource_type = attributes[:'external_resource_type']
      end

      if attributes.key?(:'user_base_role_slug')
        self.user_base_role_slug = attributes[:'user_base_role_slug']
      end

      if attributes.key?(:'user_custom_role_slugs')
        self.user_custom_role_slugs = attributes[:'user_custom_role_slugs']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          api_key_roles == o.api_key_roles &&
          external_resource_external_id == o.external_resource_external_id &&
          external_resource_type == o.external_resource_type &&
          user_base_role_slug == o.user_base_role_slug &&
          user_custom_role_slugs == o.user_custom_role_slugs
    end
    ...
    def hash
      [api_key_roles, external_resource_external_id, external_resource_type, user_base_role_slug, user_custom_role_slugs].hash
    end
