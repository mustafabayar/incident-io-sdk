  ...
  class AuditLogTargetV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::AuditLogTargetV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::AuditLogTargetV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["api_key", "alert_route", "alert_schema", "alert_source", "announcement_rule", "catalog_type", "custom_field", "debrief_invite_rule", "escalation_path", "follow_up_priority", "holiday_user_feed", "incident", "incident_call_setting", "incident_duration_metric", "incident_role", "incident_status", "incident_timestamp", "incident_type", "integration", "internal_status_page", "nudge", "schedule_override", "policy", "post_incident_task", "postmortem_template", "postmortem_template_section", "private_incident_membership", "rbac_role", "scim_group", "schedule", "severity", "status_page", "status_page_sub_page", "status_page_template", "user", "workflow"])
