  ...
  class CatalogEntryV2
    ...
    def self.openapi_types
      {
        ...
        :'archived_at' => :'Time',
        :'attribute_values' => :'Hash<String, CatalogEntryEngineParamBindingV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CatalogEntryV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CatalogEntryV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'attribute_values')
        if (value = attributes[:'attribute_values']).is_a?(Hash)
          self.attribute_values = value
        end
      else
        self.attribute_values = nil
      end
    ...
    def list_invalid_properties
      ...
      if @attribute_values.nil?
        invalid_properties.push('invalid value for "attribute_values", attribute_values cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @attribute_values.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] attribute_values Value to be assigned
    def attribute_values=(attribute_values)
      if attribute_values.nil?
        fail ArgumentError, 'attribute_values cannot be nil'
      end
    ...
    def hash
      [aliases, archived_at, attribute_values, catalog_type_id, created_at, external_id, id, name, rank, updated_at].hash
    end
