  ...
  class CatalogListEntriesResultV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CatalogListEntriesResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CatalogListEntriesResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'catalog_entries')
        if (value = attributes[:'catalog_entries']).is_a?(Array)
          self.catalog_entries = value
        end
      else
        self.catalog_entries = nil
      end
    ...
    def list_invalid_properties
      ...
      if @catalog_entries.nil?
        invalid_properties.push('invalid value for "catalog_entries", catalog_entries cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @catalog_entries.nil?
    ...
    # @param [Object] catalog_entries Value to be assigned
    def catalog_entries=(catalog_entries)
      if catalog_entries.nil?
        fail ArgumentError, 'catalog_entries cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          catalog_entries == o.catalog_entries &&
    ...
    def hash
      [catalog_entries, catalog_type, pagination_meta].hash
    end
