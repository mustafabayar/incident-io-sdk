...
module IncidentIoSdk
  class CatalogListTypesResultV2
    ...
    def self.openapi_types
      {
        :'catalog_types' => :'Array<CatalogTypeV2>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CatalogListTypesResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CatalogListTypesResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'catalog_types')
        if (value = attributes[:'catalog_types']).is_a?(Array)
          self.catalog_types = value
        end
      else
        self.catalog_types = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @catalog_types.nil?
        invalid_properties.push('invalid value for "catalog_types", catalog_types cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @catalog_types.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] catalog_types Value to be assigned
    def catalog_types=(catalog_types)
      if catalog_types.nil?
        fail ArgumentError, 'catalog_types cannot be nil'
      end
