  ...
  class CatalogUpdateEntryResultV3
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CatalogUpdateEntryResultV3` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CatalogUpdateEntryResultV3`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @catalog_entry.nil?
        invalid_properties.push('invalid value for "catalog_entry", catalog_entry cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @catalog_entry.nil?
      return false if @catalog_type.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] catalog_entry Value to be assigned
    def catalog_entry=(catalog_entry)
      if catalog_entry.nil?
        fail ArgumentError, 'catalog_entry cannot be nil'
      end
    ...
    def hash
      [catalog_entry, catalog_type].hash
    end
