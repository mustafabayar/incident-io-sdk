  ...
  class CustomFieldEntryV1
    ...
    def self.openapi_types
      {
        ...
        :'values' => :'Array<CustomFieldValueV1>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CustomFieldEntryV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CustomFieldEntryV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @custom_field.nil?
        invalid_properties.push('invalid value for "custom_field", custom_field cannot be nil.')
      end
    ...
    # @param [Object] custom_field Value to be assigned
    def custom_field=(custom_field)
      if custom_field.nil?
        fail ArgumentError, 'custom_field cannot be nil'
      end
