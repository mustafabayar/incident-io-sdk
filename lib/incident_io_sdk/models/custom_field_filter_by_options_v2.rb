...
module IncidentIoSdk
  class CustomFieldFilterByOptionsV2
    ...
    attr_accessor :catalog_attribute_id

    # This must be the ID of a custom field, which must have values of the same type as the attribute you are filtering by.  When this filtering field is set on an incident, the options for this custom field will be filtered to only those with the attribute value that matches the value of the filtering field.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CustomFieldFilterByOptionsV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CustomFieldFilterByOptionsV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'catalog_attribute_id')
        self.catalog_attribute_id = attributes[:'catalog_attribute_id']
      else
        self.catalog_attribute_id = nil
      end

      if attributes.key?(:'custom_field_id')
        self.custom_field_id = attributes[:'custom_field_id']
      else
        self.custom_field_id = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @catalog_attribute_id.nil?
        invalid_properties.push('invalid value for "catalog_attribute_id", catalog_attribute_id cannot be nil.')
      end

      if @custom_field_id.nil?
        invalid_properties.push('invalid value for "custom_field_id", custom_field_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @catalog_attribute_id.nil?
      return false if @custom_field_id.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] catalog_attribute_id Value to be assigned
    def catalog_attribute_id=(catalog_attribute_id)
      if catalog_attribute_id.nil?
        fail ArgumentError, 'catalog_attribute_id cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] custom_field_id Value to be assigned
    def custom_field_id=(custom_field_id)
      if custom_field_id.nil?
        fail ArgumentError, 'custom_field_id cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          catalog_attribute_id == o.catalog_attribute_id &&
    ...
    def hash
      [catalog_attribute_id, custom_field_id].hash
    end
