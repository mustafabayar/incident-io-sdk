  ...
  class CustomFieldOptionsListResultV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CustomFieldOptionsListResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CustomFieldOptionsListResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'custom_field_options')
        if (value = attributes[:'custom_field_options']).is_a?(Array)
          self.custom_field_options = value
        end
      else
        self.custom_field_options = nil
      end
    ...
    def list_invalid_properties
      ...
      if @custom_field_options.nil?
        invalid_properties.push('invalid value for "custom_field_options", custom_field_options cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @custom_field_options.nil?
      return false if @pagination_meta.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] custom_field_options Value to be assigned
    def custom_field_options=(custom_field_options)
      if custom_field_options.nil?
        fail ArgumentError, 'custom_field_options cannot be nil'
      end
    ...
    def hash
      [custom_field_options, pagination_meta].hash
    end
