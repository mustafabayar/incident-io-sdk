...
module IncidentIoSdk
  class CustomFieldValueV1
    attr_accessor :value_catalog_entry
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::CustomFieldValueV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::CustomFieldValueV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'value_catalog_entry')
        self.value_catalog_entry = attributes[:'value_catalog_entry']
      end
      ...
      if attributes.key?(:'value_option')
        self.value_option = attributes[:'value_option']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          value_catalog_entry == o.value_catalog_entry &&
    ...
    def hash
      [value_catalog_entry, value_link, value_numeric, value_option, value_text].hash
    end
