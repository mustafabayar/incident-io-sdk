  ...
  class EmbeddedIncidentRoleV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::EmbeddedIncidentRoleV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::EmbeddedIncidentRoleV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @shortform.nil?
        invalid_properties.push('invalid value for "shortform", shortform cannot be nil.')
      end
    ...
    def valid?
      ...
      role_type_validator = EnumAttributeValidator.new('String', ["lead", "reporter", "custom"])
      return false unless role_type_validator.valid?(@role_type)
      return false if @shortform.nil?
    ...
    def role_type=(role_type)
      validator = EnumAttributeValidator.new('String', ["lead", "reporter", "custom"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] shortform Value to be assigned
    def shortform=(shortform)
      if shortform.nil?
        fail ArgumentError, 'shortform cannot be nil'
      end
    ...
    def hash
      [created_at, description, id, instructions, name, required, role_type, shortform, updated_at].hash
    end
