...
module IncidentIoSdk
  class EscalationPathNodeLevelV2
    attr_accessor :round_robin_config
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::EscalationPathNodeLevelV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::EscalationPathNodeLevelV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'round_robin_config')
        self.round_robin_config = attributes[:'round_robin_config']
      end
      ...
      if attributes.key?(:'time_to_ack_interval_condition')
        self.time_to_ack_interval_condition = attributes[:'time_to_ack_interval_condition']
      end

      if attributes.key?(:'time_to_ack_seconds')
        self.time_to_ack_seconds = attributes[:'time_to_ack_seconds']
      end

      if attributes.key?(:'time_to_ack_weekday_interval_config_id')
        self.time_to_ack_weekday_interval_config_id = attributes[:'time_to_ack_weekday_interval_config_id']
      end
    end
    ...
    def valid?
      ...
      time_to_ack_interval_condition_validator = EnumAttributeValidator.new('String', ["active", "inactive"])
      return false unless time_to_ack_interval_condition_validator.valid?(@time_to_ack_interval_condition)
      true
    end
    ...
    # @param [Object] time_to_ack_interval_condition Object to be assigned
    def time_to_ack_interval_condition=(time_to_ack_interval_condition)
      validator = EnumAttributeValidator.new('String', ["active", "inactive"])
      unless validator.valid?(time_to_ack_interval_condition)
        fail ArgumentError, "invalid value for \"time_to_ack_interval_condition\", must be one of #{validator.allowable_values}."
      end
      @time_to_ack_interval_condition = time_to_ack_interval_condition
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          round_robin_config == o.round_robin_config &&
          targets == o.targets &&
          time_to_ack_interval_condition == o.time_to_ack_interval_condition &&
          time_to_ack_seconds == o.time_to_ack_seconds &&
          time_to_ack_weekday_interval_config_id == o.time_to_ack_weekday_interval_config_id
    end
    ...
    def hash
      [round_robin_config, targets, time_to_ack_interval_condition, time_to_ack_seconds, time_to_ack_weekday_interval_config_id].hash
    end
