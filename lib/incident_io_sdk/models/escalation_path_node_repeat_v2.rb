  ...
  class EscalationPathNodeRepeatV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::EscalationPathNodeRepeatV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::EscalationPathNodeRepeatV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'repeat_times')
        self.repeat_times = attributes[:'repeat_times']
      else
        self.repeat_times = nil
      end

      if attributes.key?(:'to_node')
    ...
    def list_invalid_properties
      ...
      if @repeat_times.nil?
        invalid_properties.push('invalid value for "repeat_times", repeat_times cannot be nil.')
      end

      if @to_node.nil?
        invalid_properties.push('invalid value for "to_node", to_node cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @repeat_times.nil?
      return false if @to_node.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] repeat_times Value to be assigned
    def repeat_times=(repeat_times)
      if repeat_times.nil?
        fail ArgumentError, 'repeat_times cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] to_node Value to be assigned
    def to_node=(to_node)
      if to_node.nil?
        fail ArgumentError, 'to_node cannot be nil'
      end
