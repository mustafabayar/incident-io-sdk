...
module IncidentIoSdk
  class EscalationPathRoundRobinConfigV2
    ...
    attr_accessor :rotate_after_seconds
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::EscalationPathRoundRobinConfigV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::EscalationPathRoundRobinConfigV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rotate_after_seconds')
        self.rotate_after_seconds = attributes[:'rotate_after_seconds']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          enabled == o.enabled &&
          rotate_after_seconds == o.rotate_after_seconds
    end
    ...
    def hash
      [enabled, rotate_after_seconds].hash
    end
