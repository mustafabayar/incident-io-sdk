  ...
  class EscalationPathTargetV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::EscalationPathTargetV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::EscalationPathTargetV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'schedule_mode')
        self.schedule_mode = attributes[:'schedule_mode']
      end
    ...
    def valid?
      ...
      schedule_mode_validator = EnumAttributeValidator.new('String', ["currently_on_call", "all_users_for_rota", "all_users", ""])
      return false unless schedule_mode_validator.valid?(@schedule_mode)
      return false if @type.nil?
      type_validator = EnumAttributeValidator.new('String', ["schedule", "user", "slack_channel"])
    ...
    # @param [Object] schedule_mode Object to be assigned
    def schedule_mode=(schedule_mode)
      validator = EnumAttributeValidator.new('String', ["currently_on_call", "all_users_for_rota", "all_users", ""])
      unless validator.valid?(schedule_mode)
        fail ArgumentError, "invalid value for \"schedule_mode\", must be one of #{validator.allowable_values}."
      end
      @schedule_mode = schedule_mode
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["schedule", "user", "slack_channel"])
    ...
    def hash
      [id, schedule_mode, type, urgency].hash
    end
