  ...
  class ExpressionNavigateOptsV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ExpressionNavigateOptsV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ExpressionNavigateOptsV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reference_label')
        self.reference_label = attributes[:'reference_label']
      else
        self.reference_label = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @reference_label.nil?
        invalid_properties.push('invalid value for "reference_label", reference_label cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @reference_label.nil?
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] reference_label Value to be assigned
    def reference_label=(reference_label)
      if reference_label.nil?
        fail ArgumentError, 'reference_label cannot be nil'
      end
    ...
    def hash
      [reference, reference_label].hash
    end
