  ...
  class ExpressionOperationPayloadV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ExpressionOperationPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ExpressionOperationPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'navigate')
        self.navigate = attributes[:'navigate']
      end
    ...
    def valid?
      ...
      operation_type_validator = EnumAttributeValidator.new('String', ["navigate", "filter", "count", "min", "max", "sum", "random", "first", "parse", "branches"])
    ...
    def operation_type=(operation_type)
      validator = EnumAttributeValidator.new('String', ["navigate", "filter", "count", "min", "max", "sum", "random", "first", "parse", "branches"])
    ...
    def hash
      [branches, filter, navigate, operation_type, parse].hash
    end
