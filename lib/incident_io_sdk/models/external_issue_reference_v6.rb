  ...
  class ExternalIssueReferenceV6
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ExternalIssueReferenceV6` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ExternalIssueReferenceV6`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'issue_permalink')
        self.issue_permalink = attributes[:'issue_permalink']
      else
        self.issue_permalink = nil
      end
    ...
    def list_invalid_properties
      ...
      if @issue_name.nil?
        invalid_properties.push('invalid value for "issue_name", issue_name cannot be nil.')
      end

      if @issue_permalink.nil?
        invalid_properties.push('invalid value for "issue_permalink", issue_permalink cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @issue_name.nil?
      return false if @issue_permalink.nil?
      return false if @provider.nil?
      provider_validator = EnumAttributeValidator.new('String', ["asana", "azure_devops", "click_up", "linear", "jira", "jira_server", "github", "gitlab", "shortcut"])
    ...
    def issue_name=(issue_name)
      if issue_name.nil?
        fail ArgumentError, 'issue_name cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] issue_permalink Value to be assigned
    def issue_permalink=(issue_permalink)
      if issue_permalink.nil?
        fail ArgumentError, 'issue_permalink cannot be nil'
      end
    ...
    def provider=(provider)
      validator = EnumAttributeValidator.new('String', ["asana", "azure_devops", "click_up", "linear", "jira", "jira_server", "github", "gitlab", "shortcut"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          issue_permalink == o.issue_permalink &&
          provider == o.provider
    end
    ...
    def hash
      [issue_name, issue_permalink, provider].hash
    end
