  ...
  class ExternalResourceV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ExternalResourceV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ExternalResourceV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      resource_type_validator = EnumAttributeValidator.new('String', ["pager_duty_incident", "opsgenie_alert", "datadog_monitor_alert", "github_pull_request", "gitlab_merge_request", "sentry_issue", "jira_issue", "atlassian_statuspage_incident", "zendesk_ticket", "google_calendar_event", "outlook_calendar_event", "slack_file", "scrubbed", "statuspage_incident"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] permalink Value to be assigned
    def permalink=(permalink)
      if permalink.nil?
        fail ArgumentError, 'permalink cannot be nil'
      end
    ...
    def resource_type=(resource_type)
      validator = EnumAttributeValidator.new('String', ["pager_duty_incident", "opsgenie_alert", "datadog_monitor_alert", "github_pull_request", "gitlab_merge_request", "sentry_issue", "jira_issue", "atlassian_statuspage_incident", "zendesk_ticket", "google_calendar_event", "outlook_calendar_event", "slack_file", "scrubbed", "statuspage_incident"])
    ...
    def hash
      [external_id, permalink, resource_type, title].hash
    end
