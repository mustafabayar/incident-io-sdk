  ...
  class FollowUpsListResultV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::FollowUpsListResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::FollowUpsListResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'follow_ups')
        if (value = attributes[:'follow_ups']).is_a?(Array)
          self.follow_ups = value
        end
      else
        self.follow_ups = nil
      end
    ...
    def list_invalid_properties
      ...
      if @follow_ups.nil?
        invalid_properties.push('invalid value for "follow_ups", follow_ups cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @follow_ups.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] follow_ups Value to be assigned
    def follow_ups=(follow_ups)
      if follow_ups.nil?
        fail ArgumentError, 'follow_ups cannot be nil'
      end
