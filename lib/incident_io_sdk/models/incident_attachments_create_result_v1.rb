...
module IncidentIoSdk
  class IncidentAttachmentsCreateResultV1
    attr_accessor :incident_attachment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentAttachmentsCreateResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentAttachmentsCreateResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_attachment')
        self.incident_attachment = attributes[:'incident_attachment']
      else
        self.incident_attachment = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_attachment.nil?
        invalid_properties.push('invalid value for "incident_attachment", incident_attachment cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_attachment.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_attachment Value to be assigned
    def incident_attachment=(incident_attachment)
      if incident_attachment.nil?
        fail ArgumentError, 'incident_attachment cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_attachment == o.incident_attachment
    end
