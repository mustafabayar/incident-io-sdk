...
module IncidentIoSdk
  class IncidentAttachmentsListResultV1
    attr_accessor :incident_attachments
    ...
    def self.openapi_types
      {
        :'incident_attachments' => :'Array<IncidentAttachmentV1>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentAttachmentsListResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentAttachmentsListResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_attachments')
        if (value = attributes[:'incident_attachments']).is_a?(Array)
          self.incident_attachments = value
        end
      else
        self.incident_attachments = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_attachments.nil?
        invalid_properties.push('invalid value for "incident_attachments", incident_attachments cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_attachments.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_attachments Value to be assigned
    def incident_attachments=(incident_attachments)
      if incident_attachments.nil?
        fail ArgumentError, 'incident_attachments cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_attachments == o.incident_attachments
    end
