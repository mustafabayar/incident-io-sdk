...
module IncidentIoSdk
  class IncidentDurationMetricWithValueV2
    attr_accessor :duration_metric

    # The calculated durations for this metric
    attr_accessor :value_seconds
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentDurationMetricWithValueV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentDurationMetricWithValueV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'duration_metric')
        self.duration_metric = attributes[:'duration_metric']
      else
        self.duration_metric = nil
      end

      if attributes.key?(:'value_seconds')
        self.value_seconds = attributes[:'value_seconds']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @duration_metric.nil?
        invalid_properties.push('invalid value for "duration_metric", duration_metric cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @duration_metric.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] duration_metric Value to be assigned
    def duration_metric=(duration_metric)
      if duration_metric.nil?
        fail ArgumentError, 'duration_metric cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          duration_metric == o.duration_metric &&
          value_seconds == o.value_seconds
    end
    ...
    def hash
      [duration_metric, value_seconds].hash
    end
