...
module IncidentIoSdk
  class IncidentMembershipsCreateResultV1
    attr_accessor :incident_membership
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentMembershipsCreateResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentMembershipsCreateResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_membership')
        self.incident_membership = attributes[:'incident_membership']
      else
        self.incident_membership = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_membership.nil?
        invalid_properties.push('invalid value for "incident_membership", incident_membership cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_membership.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_membership Value to be assigned
    def incident_membership=(incident_membership)
      if incident_membership.nil?
        fail ArgumentError, 'incident_membership cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_membership == o.incident_membership
    end
