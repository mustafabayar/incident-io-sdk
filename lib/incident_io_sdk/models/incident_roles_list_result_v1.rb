  ...
  class IncidentRolesListResultV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentRolesListResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentRolesListResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_roles')
        if (value = attributes[:'incident_roles']).is_a?(Array)
          self.incident_roles = value
        end
      else
        self.incident_roles = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_roles.nil?
        invalid_properties.push('invalid value for "incident_roles", incident_roles cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_roles.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_roles Value to be assigned
    def incident_roles=(incident_roles)
      if incident_roles.nil?
        fail ArgumentError, 'incident_roles cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_roles == o.incident_roles
    end
