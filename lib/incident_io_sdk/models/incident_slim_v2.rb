  ...
  class IncidentSlimV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentSlimV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentSlimV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'status_category')
        self.status_category = attributes[:'status_category']
      else
        self.status_category = nil
      end
    ...
    def list_invalid_properties
      ...
      if @status_category.nil?
        invalid_properties.push('invalid value for "status_category", status_category cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @status_category.nil?
      status_category_validator = EnumAttributeValidator.new('String', ["triage", "declined", "merged", "canceled", "active", "post-incident", "closed", "paused"])
      return false unless status_category_validator.valid?(@status_category)
    ...
    # @param [Object] status_category Object to be assigned
    def status_category=(status_category)
      validator = EnumAttributeValidator.new('String', ["triage", "declined", "merged", "canceled", "active", "post-incident", "closed", "paused"])
      unless validator.valid?(status_category)
        fail ArgumentError, "invalid value for \"status_category\", must be one of #{validator.allowable_values}."
      end
      @status_category = status_category
    end
    ...
    def hash
      [external_id, id, name, reference, status_category, summary, visibility].hash
    end
