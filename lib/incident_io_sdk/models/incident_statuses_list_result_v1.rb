...
module IncidentIoSdk
  class IncidentStatusesListResultV1
    attr_accessor :incident_statuses
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentStatusesListResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentStatusesListResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_statuses')
        if (value = attributes[:'incident_statuses']).is_a?(Array)
          self.incident_statuses = value
        end
      else
        self.incident_statuses = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_statuses.nil?
        invalid_properties.push('invalid value for "incident_statuses", incident_statuses cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_statuses.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_statuses Value to be assigned
    def incident_statuses=(incident_statuses)
      if incident_statuses.nil?
        fail ArgumentError, 'incident_statuses cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_statuses == o.incident_statuses
    end
