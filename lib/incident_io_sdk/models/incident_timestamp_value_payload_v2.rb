...
module IncidentIoSdk
  class IncidentTimestampValuePayloadV2
    ...
    attr_accessor :incident_timestamp_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentTimestampValuePayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentTimestampValuePayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_timestamp_id')
        self.incident_timestamp_id = attributes[:'incident_timestamp_id']
      else
        self.incident_timestamp_id = nil
      end
    ...
    def list_invalid_properties
      ...
      if @incident_timestamp_id.nil?
        invalid_properties.push('invalid value for "incident_timestamp_id", incident_timestamp_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_timestamp_id.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_timestamp_id Value to be assigned
    def incident_timestamp_id=(incident_timestamp_id)
      if incident_timestamp_id.nil?
        fail ArgumentError, 'incident_timestamp_id cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_timestamp_id == o.incident_timestamp_id &&
          value == o.value
    end
    ...
    def hash
      [incident_timestamp_id, value].hash
    end
