...
module IncidentIoSdk
  class IncidentTimestampWithValueV2
    attr_accessor :incident_timestamp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentTimestampWithValueV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentTimestampWithValueV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_timestamp')
        self.incident_timestamp = attributes[:'incident_timestamp']
      else
        self.incident_timestamp = nil
      end
    ...
    def list_invalid_properties
      ...
      if @incident_timestamp.nil?
        invalid_properties.push('invalid value for "incident_timestamp", incident_timestamp cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_timestamp.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_timestamp Value to be assigned
    def incident_timestamp=(incident_timestamp)
      if incident_timestamp.nil?
        fail ArgumentError, 'incident_timestamp cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_timestamp == o.incident_timestamp &&
          value == o.value
    end
