...
module IncidentIoSdk
  class IncidentTimestampsListResultV2
    attr_accessor :incident_timestamps
    ...
    def self.openapi_types
      {
        :'incident_timestamps' => :'Array<IncidentTimestampV2>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentTimestampsListResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentTimestampsListResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_timestamps')
        if (value = attributes[:'incident_timestamps']).is_a?(Array)
          self.incident_timestamps = value
        end
      else
        self.incident_timestamps = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_timestamps.nil?
        invalid_properties.push('invalid value for "incident_timestamps", incident_timestamps cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_timestamps.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_timestamps Value to be assigned
    def incident_timestamps=(incident_timestamps)
      if incident_timestamps.nil?
        fail ArgumentError, 'incident_timestamps cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_timestamps == o.incident_timestamps
    end
