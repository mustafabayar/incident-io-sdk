...
module IncidentIoSdk
  class IncidentTypesListResultV1
    ...
    def self.openapi_types
      {
        :'incident_types' => :'Array<IncidentTypeV1>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentTypesListResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentTypesListResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_types')
        if (value = attributes[:'incident_types']).is_a?(Array)
          self.incident_types = value
        end
      else
        self.incident_types = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @incident_types.nil?
        invalid_properties.push('invalid value for "incident_types", incident_types cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @incident_types.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] incident_types Value to be assigned
    def incident_types=(incident_types)
      if incident_types.nil?
        fail ArgumentError, 'incident_types cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_types == o.incident_types
    end
