  ...
  class IncidentTypesShowResultV1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentTypesShowResultV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentTypesShowResultV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @incident_type.nil?
        invalid_properties.push('invalid value for "incident_type", incident_type cannot be nil.')
      end
    ...
    # @param [Object] incident_type Value to be assigned
    def incident_type=(incident_type)
      if incident_type.nil?
        fail ArgumentError, 'incident_type cannot be nil'
      end
