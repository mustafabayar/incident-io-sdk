...
module IncidentIoSdk
  class IncidentUpdatesListResultV2
    attr_accessor :incident_updates
    ...
    def self.openapi_types
      {
        :'incident_updates' => :'Array<IncidentUpdateV2>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentUpdatesListResultV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentUpdatesListResultV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'incident_updates')
        if (value = attributes[:'incident_updates']).is_a?(Array)
          self.incident_updates = value
        end
      else
        self.incident_updates = nil
      end
    ...
    # @param [Object] incident_updates Value to be assigned
    def incident_updates=(incident_updates)
      if incident_updates.nil?
        fail ArgumentError, 'incident_updates cannot be nil'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          incident_updates == o.incident_updates &&
    ...
    def hash
      [incident_updates, pagination_meta].hash
    end
