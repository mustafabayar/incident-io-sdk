  ...
  class IncidentV1
    ...
    def self.openapi_types
      {
        ...
        :'status' => :'String',
        :'summary' => :'String',
        :'timestamps' => :'Array<IncidentTimestampValueV1>',
        :'updated_at' => :'Time',
        :'visibility' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentV1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentV1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      mode_validator = EnumAttributeValidator.new('String', ["real", "test", "tutorial"])
    ...
    def mode=(mode)
      validator = EnumAttributeValidator.new('String', ["real", "test", "tutorial"])
    ...
    def hash
      [call_url, created_at, creator, custom_field_entries, id, incident_role_assignments, incident_type, mode, name, permalink, postmortem_document_url, reference, severity, slack_channel_id, slack_channel_name, slack_team_id, status, summary, timestamps, updated_at, visibility].hash
    end
