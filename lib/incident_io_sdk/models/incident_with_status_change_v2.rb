  ...
  class IncidentWithStatusChangeV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::IncidentWithStatusChangeV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::IncidentWithStatusChangeV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @previous_status.nil?
        invalid_properties.push('invalid value for "previous_status", previous_status cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] new_status Value to be assigned
    def new_status=(new_status)
      if new_status.nil?
        fail ArgumentError, 'new_status cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] previous_status Value to be assigned
    def previous_status=(previous_status)
      if previous_status.nil?
        fail ArgumentError, 'previous_status cannot be nil'
      end
    ...
    def hash
      [incident, message, new_status, previous_status].hash
    end
