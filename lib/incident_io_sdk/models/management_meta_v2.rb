  ...
  class ManagementMetaV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ManagementMetaV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ManagementMetaV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @managed_by.nil?
        invalid_properties.push('invalid value for "managed_by", managed_by cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @managed_by.nil?
      managed_by_validator = EnumAttributeValidator.new('String', ["dashboard", "terraform", "external"])
    ...
    def managed_by=(managed_by)
      validator = EnumAttributeValidator.new('String', ["dashboard", "terraform", "external"])
    ...
    def hash
      [annotations, managed_by, source_url].hash
    end
