...
module IncidentIoSdk
  class ScheduleConfigV2
    ...
    def self.openapi_types
      {
        :'rotations' => :'Array<ScheduleRotationV2>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ScheduleConfigV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ScheduleConfigV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'rotations')
    ...
    def list_invalid_properties
      ...
      if @rotations.nil?
        invalid_properties.push('invalid value for "rotations", rotations cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @rotations.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] rotations Value to be assigned
    def rotations=(rotations)
      if rotations.nil?
        fail ArgumentError, 'rotations cannot be nil'
      end
