...
module IncidentIoSdk
  class ScheduleEntriesListPayloadV2
    ...
    def self.openapi_types
      {
        :'final' => :'Array<ScheduleEntryV2>',
        :'overrides' => :'Array<ScheduleEntryV2>',
        :'scheduled' => :'Array<ScheduleEntryV2>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ScheduleEntriesListPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ScheduleEntriesListPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'final')
        if (value = attributes[:'final']).is_a?(Array)
          self.final = value
        end
      else
        self.final = nil
      ...
      if attributes.key?(:'scheduled')
        if (value = attributes[:'scheduled']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if @final.nil?
        invalid_properties.push('invalid value for "final", final cannot be nil.')
      end
    ...
    # @param [Object] final Value to be assigned
    def final=(final)
      if final.nil?
        fail ArgumentError, 'final cannot be nil'
      end
    ...
    def hash
      [final, overrides, scheduled].hash
    end
