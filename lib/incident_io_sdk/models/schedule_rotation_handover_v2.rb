  ...
  class ScheduleRotationHandoverV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ScheduleRotationHandoverV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ScheduleRotationHandoverV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      interval_type_validator = EnumAttributeValidator.new('String', ["hourly", "daily", "weekly"])
      return false unless interval_type_validator.valid?(@interval_type)
      true
    end
    ...
    def interval_type=(interval_type)
      ...
      unless validator.valid?(interval_type)
        fail ArgumentError, "invalid value for \"interval_type\", must be one of #{validator.allowable_values}."
      end
      @interval_type = interval_type
    end
