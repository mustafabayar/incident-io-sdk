  ...
  class ScheduleRotationWorkingIntervalUpdatePayloadV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::ScheduleRotationWorkingIntervalUpdatePayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::ScheduleRotationWorkingIntervalUpdatePayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      weekday_validator = EnumAttributeValidator.new('String', ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"])
      return false unless weekday_validator.valid?(@weekday)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] weekday Object to be assigned
    def weekday=(weekday)
      validator = EnumAttributeValidator.new('String', ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"])
      unless validator.valid?(weekday)
        fail ArgumentError, "invalid value for \"weekday\", must be one of #{validator.allowable_values}."
      end
      @weekday = weekday
    end
    ...
    def hash
      [end_time, start_time, weekday].hash
    end
