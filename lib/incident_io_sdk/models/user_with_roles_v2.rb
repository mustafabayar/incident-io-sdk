  ...
  class UserWithRolesV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::UserWithRolesV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::UserWithRolesV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'base_role')
        self.base_role = attributes[:'base_role']
      else
        self.base_role = nil
      end
      ...
      if attributes.key?(:'slack_user_id')
        self.slack_user_id = attributes[:'slack_user_id']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @base_role.nil?
        invalid_properties.push('invalid value for "base_role", base_role cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @base_role.nil?
      return false if @custom_roles.nil?
      return false if @id.nil?
      return false if @name.nil?
      return false if @role.nil?
      role_validator = EnumAttributeValidator.new('String', ["viewer", "responder", "administrator", "owner", "unset"])
      return false unless role_validator.valid?(@role)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] base_role Value to be assigned
    def base_role=(base_role)
      if base_role.nil?
        fail ArgumentError, 'base_role cannot be nil'
      end
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["viewer", "responder", "administrator", "owner", "unset"])
    ...
    def hash
      [base_role, custom_roles, email, id, name, role, slack_user_id].hash
    end
