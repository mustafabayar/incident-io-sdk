  ...
  class WorkflowsUpdateWorkflowPayloadV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IncidentIoSdk::WorkflowsUpdateWorkflowPayloadV2` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IncidentIoSdk::WorkflowsUpdateWorkflowPayloadV2`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [annotations, condition_groups, continue_on_step_error, delay, expressions, folder, include_private_incidents, name, once_for, runs_on_incident_modes, runs_on_incidents, shortform, state, steps].hash
    end
